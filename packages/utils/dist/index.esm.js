...
const getLanguage = () => (NAVIGATOR.browserLanguage || NAVIGATOR.language)?.replace(/-\w+/g, (word) => word.toUpperCase());
...
class UaBrowser {
    ...
    getChromeVars = () => this.ua.replace(/^.*Chrome\/([\d]+).*$/, '$1');
    setUA(ua) {
        if (typeof ua === 'string') {
            this.ua = ua;
        }
...
export { browser, cloneDeep, copy, getQueryStringArgs, isObject$1 as isObject, request };
